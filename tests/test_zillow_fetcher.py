...
class TestSplitLocations:
    """Unit tests for split_locations helper."""

    def test_split_locations_returns_all_when_limit_none(self):
        raw = (
            "37206, Nashville, TN; 37216, Nashville, TN; 37209, Nashville, TN; "
            "Midtown, Nashville, TN; Brentwood, TN; Nashville, TN"
        )
    ...
    def test_split_locations_empty_string(self):
        assert split_locations("", limit=None) == []

    def test_split_locations_trims_whitespace_and_ignores_empty_tokens(self):
        raw = "  a  ; ;  b ;  c ;"
        result = split_locations(raw, limit=None)
        assert result == ["a", "b", "c"]

    def test_split_locations_default_behavior_same_as_previous_limit(self):
